...
for i in range(tuning_length):
    ...
    at_inputs = torch.cat((at_inputs, o.reshape(1, num_observed_features, num_input_dimensions)), 0)
    obs_count += 1

    bm = binder.scale_binding_matrix(Bs[i])
    x_B = binder.bind(o, bm[:-1])
...
while obs_count < num_frames:
    ...
    for cycle in range(tuning_cycles):
        ...
        with torch.no_grad():
            ...
            upd_B = binder.update_binding_matrix_nxm_(Bs[0], grad_B, at_learning_rate, bm_momentum)

            
            # Compare binding matrix to ideal matrix
            c_bm = binder.scale_binding_matrix(upd_B)
            mat_loss = evaluator.FBE_nxm(c_bm, ideal_binding, index_additional_features)
        ...
        for i in range(tuning_length):
            ...
            bm = binder.scale_binding_matrix(Bs[i])
            x_B = binder.bind(at_inputs[i], bm[:-1])
            x = preprocessor.convert_data_AT_to_LSTM(x_B)
...
############################################################################
##########  EVALUATION #####################################################
pred_errors = evaluator.prediction_errors_nxm(
    observations, 
